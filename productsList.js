...
divHeader.setAttribute('class', `flex w-full text-gray-900 text-4xl text-bold justify-center pt-10`);
divHeader.setAttribute('style', "font-family: 'Prompt', sans-serif; font-size: 3em;");
divHeader.textContent = 'iPhone รุ่นไหนที่ใช่สำหรับคุณ';

// products
const divProducts = document.querySelector('#products');
divProducts.setAttribute('class', `mt-6 grid grid-cols-1 gap-y-10 gap-x-6 sm:grid-cols-2 lg:grid-cols-4 xl:gap-x-8`);
...
function showProducts() {
    for (const pd of products) {
        ...
        divImage.setAttribute('class', 'w-full min-h-80 transition duration-200 aspect-w-1 aspect-h-1 rounded-md overflow-hidden group-hover:opacity-80 lg:h-80 lg:aspect-none cursor-pointer');
        let imgElem = document.createElement('img');
        imgElem.setAttribute('src', pd.product_image);
        imgElem.setAttribute('class', 'w-full h-full object-center object-cover lg:w-full lg:h-full');
        ...
        if (pd.stock > 0) {
            ...
            add.setAttribute('class', 'mt-0 w-full bg-blue-500 transition duration-200 border border-transparent rounded-2xl py-3 px-3 flex items-center justify-center text-base font-sm text-white hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500');
            add.textContent = 'Add to Cart';
            add.addEventListener('click', addProductToCart);
        } else {
            add.setAttribute('class', 'mt-0 w-full bg-gray-300 rounded-2xl py-3 px-3 flex items-center justify-center text-base font-sm text-white');
